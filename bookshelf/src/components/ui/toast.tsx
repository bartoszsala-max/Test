...
>(({ className, variant = "default", ...props }, ref) => (
  <ToastPrimitive.Root
    ...
    className={cn(
      ...
      variant === "default" && "bg-ivory dark:bg-charcoal border-charcoal/10 dark:border-ivory/10 text-charcoal dark:text-ivory",
      variant === "success" && "bg-teal text-white border-teal",
      variant === "error" && "bg-red-500 text-white border-red-500",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Close
    ref={ref}
    className={cn("rounded-md p-1 opacity-60 transition-opacity hover:opacity-100 focus:outline-none", className)}
    {...props}
  >
    <X className="h-4 w-4" />
  </ToastPrimitive.Close>
