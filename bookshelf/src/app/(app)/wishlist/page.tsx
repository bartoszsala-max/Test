...
export default function WishlistPage() {
  ...
  const years = Array.from(
    new Set(books.map((b) => b.wishlist_year ?? 0))
  ).sort((a, b) => a - b);
          ...
          return (
            ...
            >
              ...
              {count > 0 && (
                <span className={cn(
                  "ml-1.5 text-xs",
                  selectedYear === year ? "text-white/80" : "text-charcoal/40 dark:text-ivory/40"
                )}>
                  {count}
                </span>
              )}
          ...
          {filteredBooks.map((book) => (
            <div key={book.id} className="group book-card">
              ...
              <div className="mt-2 px-0.5">
                <p className="text-sm font-medium text-charcoal dark:text-ivory leading-snug line-clamp-2">
                  {book.title}
                </p>
