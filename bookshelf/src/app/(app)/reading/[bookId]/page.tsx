  ...
  if (!book) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/reading">
          <Button variant="ghost" className="mt-4">← Back to Reading</Button>
        </Link>
      </div>
    );
  }
      ...
      <div className="relative rounded-2xl overflow-hidden mb-6">
        ...
        <div className="relative bg-gradient-to-r from-charcoal/5 to-transparent dark:from-charcoal/40 rounded-2xl p-6 flex gap-6 items-end">
          <BookCover
            src={book.cover_url}
            title={book.title}
            className="w-28 h-40 shrink-0 shadow-2xl"
            priority
          />
          <div className="flex-1 min-w-0 pb-1">
            ...
            <h1 className="font-serif text-2xl sm:text-3xl font-bold text-charcoal dark:text-ivory leading-tight mb-1">
              {book.title}
            </h1>
            <p className="text-charcoal/70 dark:text-ivory/70 text-sm mb-2">
      ...
      <Tabs defaultValue="excerpts">
        <TabsList className="mb-5">
          <TabsTrigger value="excerpts">
            Excerpts <span className="ml-1.5 text-xs opacity-60">{excerpts.length}</span>
          </TabsTrigger>
          <TabsTrigger value="thoughts">
            Thoughts <span className="ml-1.5 text-xs opacity-60">{thoughts.length}</span>
          </TabsTrigger>
        ...
        <TabsContent value="excerpts" className="space-y-4">
          ...
          <div className="rounded-xl border border-charcoal/10 dark:border-ivory/10 bg-white dark:bg-charcoal/40 p-4 space-y-3">
            <Textarea
              placeholder="Paste or type an excerpt…"
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                disabled={!newExcerptText.trim() || addingExcerpt}
                size="sm"
              >
            ...
            <p className="text-xs text-charcoal/40 dark:text-ivory/40">
              Or upload a photo of a page — AI will extract the text automatically.
            </p>
          </div>

          {excerpts.length === 0 ? (
            <p className="text-sm text-center text-charcoal/40 dark:text-ivory/40 py-8">
              No excerpts yet. Save your first passage above.
            </p>
          ) : (
            <div className="space-y-3">
              {excerpts.map((e) => (
        ...
        <TabsContent value="thoughts" className="space-y-4">
          <div className="rounded-xl border border-charcoal/10 dark:border-ivory/10 bg-white dark:bg-charcoal/40 p-4 space-y-3">
            <Textarea
              placeholder="Write a personal thought or note… (markdown supported)"
              value={newThought}
            ...
            <Button
              ...
              disabled={!newThought.trim() || addingThought}
              size="sm"
            >
          ...
          {thoughts.length === 0 ? (
            <p className="text-sm text-center text-charcoal/40 dark:text-ivory/40 py-8">
              No thoughts yet. Capture your impressions above.
            </p>
          ) : (
            <div className="space-y-3">
              {thoughts.map((t) => (
                <div
                  key={t.id}
                  className="group rounded-xl border border-charcoal/10 dark:border-ivory/10 bg-white dark:bg-charcoal/40 p-4"
                >
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-sm text-charcoal dark:text-ivory leading-relaxed whitespace-pre-wrap flex-1">
                      {t.content}
                    </p>
                    <Button
                      size="icon-sm"
                      variant="ghost"
                      className="opacity-0 group-hover:opacity-100 transition-opacity text-red-400 shrink-0"
