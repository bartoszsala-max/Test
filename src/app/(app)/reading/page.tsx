    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <h3 className="font-serif text-xl font-medium text-charcoal/60 dark:text-ivory/60">No books in progress</h3>
          <p className="text-sm text-charcoal/40 dark:text-ivory/40 mt-1">Promote a book from your wishlist to start reading.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {books.map((book) => {
            ...
            return (
              <div key={book.id} className="group relative flex flex-col bg-white dark:bg-charcoal/60 rounded-2xl border border-charcoal/8 dark:border-ivory/8 shadow-sm overflow-hidden book-card">
                <Link href={`/reading/${book.id}`} className="relative h-48 shrink-0 overflow-hidden">
                  {book.cover_url ? (
                    <div className="absolute inset-0 bg-cover bg-center blur-xl scale-110 opacity-40" style={{ backgroundImage: `url(${book.cover_url})` }} />
                  ) : (
                    <div className="absolute inset-0 bg-gradient-to-br from-sienna/20 to-teal/20" />
                  )}
                  <div className="absolute inset-0 flex items-center justify-center">
                    <BookCover src={book.cover_url} title={book.title} className="h-36 w-24 shadow-xl" />
                  </div>
                </Link>
                <div className="flex flex-col flex-1 p-4">
                  <Link href={`/reading/${book.id}`} className="flex-1">
                    <h3 className="font-serif text-lg font-semibold text-charcoal dark:text-ivory leading-snug line-clamp-2 hover:text-sienna transition-colors">{book.title}</h3>
                    <p className="text-sm text-charcoal/60 dark:text-ivory/60 mt-0.5 truncate">{book.authors?.join(", ") ?? "Unknown author"}</p>
                  </Link>
                  <div className="flex items-center justify-between mt-4">
                    ...
                    <Button size="sm" variant="secondary" onClick={() => setFinishBook(book)} className="text-xs">
      ...
      <FinishBookDialog book={finishBook} onOpenChange={(open) => !open && setFinishBook(null)} onSuccess={fetchBooks} />
    </div>
  );
}
