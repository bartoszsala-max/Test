  ...
  const stream = new ReadableStream({
    async start(controller) {
      const aiStream = await client.messages.stream({
        ...
        max_tokens: 512,
        messages: [{ role: "user", content: `Condense the following book excerpt into 2-4 clear, essential bullet points capturing the key ideas:\n\n"${text}"\n\nFormat as bullet points starting with \u2022` }],
      });
      let fullText = "";
      ...
      if (excerptId) { await supabase.from("excerpts").update({ ai_summary: fullText }).eq("id", excerptId); }
      controller.close();
    },
  });
